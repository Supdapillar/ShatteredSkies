...
{
    public class BasicEnemyAlly : Ally
    {

        public Vector2 GotoPos;
        public bool GoLeft = true; // 0 is left // 1 is right

        public BasicEnemyAlly(Vector2 PS, SceneManager Scenemana, Player createdby) : base(PS, Scenemana, createdby)
        {
            Pos = PS;
        ...
        public override void Update(GameTime GT)
        {
            ...
            if (ShotDelay <= 0)
            {
                SceneMan.Bullets.Add(new BasicShotWeak(0,new Vector2(Pos.X + (float)SceneMan.rand.NextDouble() + 1.5f, Pos.Y -4), SceneMan, this)); //Bullets
        ...
        public override void Draw(SpriteBatch sb)
        {
            ...
            sb.Draw(SceneMan.Textures["BasicEnemyOutline"], new Rectangle((int)Math.Ceiling(Pos.X), (int)Math.Ceiling(Pos.Y), (int)WidthHeight.X, (int)WidthHeight.Y), new Rectangle(0, 0, (int)WidthHeight.X, (int)WidthHeight.Y), Color.White, 0f, new Vector2(0, 0), SpriteEffects.FlipVertically, 0.33f);
            sb.Draw(SceneMan.Textures["BasicEnemyInside"], new Rectangle((int)Math.Ceiling(Pos.X), (int)Math.Ceiling(Pos.Y), (int)WidthHeight.X, (int)WidthHeight.Y), new Rectangle(0, 0, (int)WidthHeight.X, (int)WidthHeight.Y), SceneMan.RelicsColors1[3], 0f, new Vector2(0, 0), SpriteEffects.FlipVertically, 0.33f);
