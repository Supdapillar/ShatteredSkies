...
{
    public class WarpRule : SpawnRule
    {
        //level spawner stuff
        //Endless spawner stuff

        public WarpRule(SceneManager Sceneman)
        {
            SceneMan = Sceneman;
            SpawnId = 1;
        }
        public override bool Spawn(int id, Vector2 Pos)
        {
            if (SpawnId==id)
            {
                SceneMan.LevelSpawner.ERelicList.Add(new Warp(SceneMan));
                return true;
            }
            return false;
        }
        ...
        public override bool EndlessSpawn(int id, Vector2 Pos)
        {
            if (id == SpawnId)
            {
                ...
                for (int i = 0; i < rand; i++)
                {
                    ...
                    SceneMan.EndlessSpawner.SpawnRules[randEne].RelicPool.Add(new Warp(SceneMan));
                }
            }
            return false;
        }
