...
namespace ShatteredSkies.Classes
{
    public class HomingCore : Enemy
    {
        ...
        public HomingCore(Vector2 PS,Overseer hostOverseer,int restingposition, SceneManager Scenemana) : base(PS, Scenemana)
        {
            Pos = PS;
            SceneMan = Scenemana;
        ...
        public override void Update(GameTime GT)
        {
            ...
            while (RotationAngle >= 135 || RotationAngle <= -135)
            {
                ...
                else if (RotationAngle <= -135)
                {
                    ...
                    TargetAngle = Helper.GetRadiansOfTwoPoints(new Vector2(Pos.X + 6, Pos.Y + 13), new Vector2(TargetedPlayer.Pos.X + TargetedPlayer.AllCores[TargetedPlayer.CurrentShipParts[0]].Width / 2, TargetedPlayer.Pos.Y + TargetedPlayer.AllCores[TargetedPlayer.CurrentShipParts[0]].Height / 2));
                    SceneMan.EnemyBullets.Add(new EnemyBasicShot(new Vector2(Pos.X + 6, Pos.Y + 13), new Vector2((float)Math.Cos(TargetAngle), (float)Math.Sin(TargetAngle)), this, SceneMan)); //Bullets
            }
        ...
        public override void Draw(SpriteBatch sb)
        {
            ...
            sb.Draw(SceneMan.Textures["HomingCore"], new Vector2((int)Pos.X + 7, (int)Pos.Y+7), new Rectangle(0, 0, (int)WidthHeight.X, (int)WidthHeight.Y), Color.White, (float)Helper.ConvertDegreesToRadians((double)RotationAngle), new Vector2(7, 7), 1, SpriteEffects.None, 0.33f);
