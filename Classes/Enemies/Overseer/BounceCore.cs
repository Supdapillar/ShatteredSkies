...
namespace ShatteredSkies.Classes
{
    public class BounceCore : Enemy
    {
        ...
        public BounceCore(Vector2 PS,Overseer hostOverseer,int restingposition, SceneManager Scenemana) : base(PS, Scenemana)
        {
            Pos = PS;
            SceneMan = Scenemana;
        ...
        public override void Update(GameTime GT)
        {
            ...
            while (RotationAngle >= 135 || RotationAngle <= -135)
            {
                ...
                {
                    RotationAngle -= 90;
                    SceneMan.EnemyBullets.Add(new BouncyShot(new Vector2(Pos.X + 6, Pos.Y + 14), new Vector2(((float)SceneMan.rand.NextDouble() * 2) - 0.5f, 1), this, SceneMan)); //Bullets
                }
                else if (RotationAngle <= -135)
                {
                    RotationAngle += 90;
                    SceneMan.EnemyBullets.Add(new BouncyShot(new Vector2(Pos.X + 6, Pos.Y + 14), new Vector2(((float)SceneMan.rand.NextDouble() * 2) - 1.5f, 1),this, SceneMan)); //Bullets
                }
            }
        ...
        public override void Draw(SpriteBatch sb)
        {
            ...
            sb.Draw(SceneMan.Textures["BounceCore"], new Vector2((int)Pos.X + 7, (int)Pos.Y+7), new Rectangle(0, 0, (int)WidthHeight.X, (int)WidthHeight.Y), Color.White, (float)Helper.ConvertDegreesToRadians((double)RotationAngle), new Vector2(7, 7), 1, SpriteEffects.None, 0.33f);
